# Citation Prediction Replication Setup Instructions

We provide scripts to reproduce our results for training a multi-turn citation prediction agent using the dataset and recipe from [Citation Prediction].
...
## Prepare Datasets
```bash
local_dir=~/data/citation_prediction
uv run --isolated examples/citation_prediction/citation_prediction_dataset.py --local_dir $local_dir
```
...
local_dir=~/data/citation_prediction
python examples/citation_prediction/citation_prediction_download.py --local_dir $local_dir
...
bash examples/citation_prediction/retriever/retrieval_launch.sh > retrieval_server.log
```

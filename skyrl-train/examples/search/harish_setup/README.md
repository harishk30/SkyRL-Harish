# Harish's Search-R1 Setup for Della Cluster

Scripts for training Search-R1 on Princeton's Della cluster with data on `/scratch/gpfs/ZHUANGL/hk4638`.
...
bash examples/search/harish_setup/setup_scratch.sh

# Step 2: Create retriever conda environment
bash examples/search/harish_setup/setup_retriever.sh
```
...
| Item | Size | Location |
|------|------|----------|
| Training data | ~2 GB | `data/searchR1/train.parquet` |
| Validation data | ~100 MB | `data/searchR1/validation.parquet` |
| FAISS index | ~60 GB | `data/searchR1/e5_Flat.index` |
| Wikipedia corpus | ~70 GB | `data/searchR1/wiki-18.jsonl` |
...
├── data/searchR1/           # All Search-R1 data

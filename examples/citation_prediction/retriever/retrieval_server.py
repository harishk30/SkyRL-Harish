...
def arxiv_id_to_yymm(arxiv_id: str) -> int:
    """Parse YYMM from an arxiv ID (e.g., '2103.12345' -> 2103). Returns 0 if unparseable."""
...
class DenseRetriever(BaseRetriever):
    def __init__(self, config):
        ...
        # Build date index: corpus index -> YYMM int (0 = old-format/unparseable)
        print("Building date index from corpus IDs...")
        self.date_index = np.array(
            [arxiv_id_to_yymm(doc["contents"].split("]")[0].lstrip("[arxiv:"))
             if doc["contents"].startswith("[arxiv:") else 0
             for doc in self.corpus],
            dtype=np.int32,
        )
        num_dated = np.count_nonzero(self.date_index)
        print(f"Date index built: {num_dated}/{len(self.date_index)} docs have parseable dates")

    def _search(self, query: str, num: int = None, return_score: bool = False, max_date: Optional[int] = None):
        if num is None:
            num = self.topk
        query_emb = self.encoder.encode(query)

        if max_date is not None:
            # Over-retrieve then filter by date
            fetch_k = min(num * 3, self.index.ntotal)
            scores, idxs = self.index.search(query_emb, k=fetch_k)
            idxs = idxs[0]
            scores = scores[0]
            # Keep docs with date <= max_date, or date == 0 (old-format/unparseable)
            mask = np.array([(self.date_index[idx] <= max_date or self.date_index[idx] == 0) if idx >= 0 else False for idx in idxs])
            idxs = idxs[mask][:num]
            scores = scores[mask][:num]
        else:
        ...
        if return_score:
            return results, scores.tolist() if hasattr(scores, 'tolist') else scores
        else:
            return results
...
def retrieve_endpoint(request: QueryRequest):
    ...
    if request.max_date is not None:
        kwargs["max_date"] = request.max_date

    if request.return_scores:
        results, scores = retriever.search(query=request.query, num=request.topk, return_score=True, **kwargs)
    else:
        results = retriever.search(query=request.query, num=request.topk, return_score=False, **kwargs)
        scores = None

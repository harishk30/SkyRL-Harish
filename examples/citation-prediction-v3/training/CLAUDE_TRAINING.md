...
**Advantages — The Most Diagnostic Metric**:
- `avg_raw_advantages` (signed mean): ±0.02 in failed runs — positive and negative cancel across the batch, net gradient is random noise. **If dynamic sampling pushes this to ±0.05+, learning should start.** If it stays at ±0.02, the non-zero groups themselves have contradictory gradients → need curriculum or SFT.
- `avg_raw_advantages_abs`: 0.28-0.43 — per-sample signal EXISTS within groups. Should stay similar or increase with dynamic sampling.
...
**If `avg_raw_advantages` (signed) increases to ±0.05+ AND reward trends upward:**
Dynamic sampling worked. Continue training longer (24hr).

**If `avg_raw_advantages` (signed) increases to ±0.05+ BUT reward stays flat:**
...
**If `avg_raw_advantages` (signed) stays at ±0.02:**
Dynamic sampling didn't fix the core issue. Move to:

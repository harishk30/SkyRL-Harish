...
def iclr_year_to_cutoff(year: int) -> int:
    """ICLR 20XX -> cutoff YYMM = October of year before (submission deadline).
    E.g., ICLR 2022 -> 2110 (Oct 2021)."""
    return (year - 2001) * 100 + 10
...
def build_skyrl_row(entry: dict, idx: int, include_intro: bool = True,
                    prompt_version: str = "v1", year_lookup: dict = None) -> dict:
    ...
    # Compute temporal cutoff from ICLR year
    max_date = None
    if year_lookup is not None:
        year = year_lookup.get(entry["paper_id"])
        if year is not None:
            max_date = iclr_year_to_cutoff(year)
    ...
    # Top-level column flows into env_extras via dataset.py
...
def main():
    ...
    parser.add_argument("--metadata_csv", type=str, default=None,
                        help="Path to massive_metadata.csv for temporal filtering (adds max_date column)")
    ...
    # Load year lookup from metadata CSV
    year_lookup = None
    if args.metadata_csv:
        print(f"Loading year lookup from {args.metadata_csv}...")
        year_lookup = {}
        with open(args.metadata_csv, "r") as f:
            reader = csv.DictReader(f)
            for row in reader:
                pid = row.get("submission_id") or row.get("paper_id")
                year_str = row.get("year")
                if pid and year_str:
                    try:
                        year_lookup[pid] = int(year_str)
                    except ValueError:
                        pass
        print(f"  Loaded {len(year_lookup)} paper->year mappings")
    ...
    for split_name, split_entries in by_split.items():
        rows = [build_skyrl_row(e, i, include_intro=include_intro, prompt_version=prompt_version, year_lookup=year_lookup) for i, e in enumerate(split_entries)]
